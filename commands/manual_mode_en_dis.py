...
def manual_enable_command(message):
    ...
    if groups[message.chat.id].enabled == False:
        ...
        bot.reply_to(
            message, groups[message.chat.id].templates[language_code]["manual_enabled.txt"], parse_mode="HTML"
        )


# --- Manual mode disable ---
@bot.message_handler(commands=["disable_manual"], func=time_filter and member_filter)
@error_handler
def manual_disable_command(message):
    ...
    if message.chat.id > 0:
        ...
        bot.reply_to(message, groups[message.chat.id].templates[language_code]["manual_disabled_in_private.txt"])
        return
    ...
    bot.reply_to(message, groups[message.chat.id].templates[language_code]["manual_disabled.txt"])
