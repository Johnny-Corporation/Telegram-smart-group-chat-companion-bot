...
def enter_promocode_reply_handler(inner_message):
    ...
    if len(code_back) == 1:
        ...
        bot.send_message(inner_message.chat.id, groups[inner_message.chat.id].templates[language_code]["more_100000_messages.txt"].format(messages=groups[inner_message.chat.id].messages_limit))
