...
def enter_new_tokens_reply_handler(inner_message):
    ...
    try:
        ...
        bot.send_message(
            inner_message.chat.id, templates[language_code]["new_tokens_calnceled.txt"]
        )
        return
    ...
    if groups[inner_message.chat.id].subscription == "Free":
        ...
        pay = accept_payment(inner_message, templates[language_code]["tokens_buy_text.txt"].format(val=val,sub="Free"), val*0.0004) 

    elif groups[inner_message.chat.id].subscription == "USER":

        pay = accept_payment(inner_message, templates[language_code]["tokens_buy_text.txt"].format(val=val,sub="USER"), val*0.0003)

    elif groups[inner_message.chat.id].subscription == "SMALL BUSINESS":

        pay = accept_payment(inner_message, templates[language_code]["tokens_buy_text.txt"].format(val=val,sub="SMALL BUSINESS"), val*0.00025)

    elif groups[inner_message.chat.id].subscription == "BIG BUSINESS":

        pay = accept_payment(inner_message, templates[language_code]["tokens_buy_text.txt"].format(val=val,sub="BIG BUSINESS"), val*0.00023)
    else:
    ...
    if pay:
        ...
        groups[inner_message.chat.id].tokens_limit = groups[inner_message.chat.id].tokens_limit + val
        ...
        bot.send_message(
            inner_message.chat.id,
            templates[language_code]["new_tokens.txt"],
            parse_mode="HTML",
        )
    else:
