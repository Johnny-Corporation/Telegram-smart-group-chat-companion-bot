...
def read_from_link(link: str) -> str:
    ...
    try:
        all_visible_text = extract_text(
            remove_scripts_and_styles(response.content.decode())
        )
    except:  # failed to decode in utf-8
        all_visible_text = "no content on text on the page"
